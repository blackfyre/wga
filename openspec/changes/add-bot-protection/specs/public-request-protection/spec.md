...
In staging and production, the system SHALL require database-backed public catalogue requests to match the configured public hostname and carry valid origin authentication applied by Cloudflare. Host matching alone SHALL NOT authenticate an edge request. Operational health checks, crawler guidance, sitemap files, and static assets SHALL remain available through the deployment hostname.
