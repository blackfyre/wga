...
- Adopt the complete non-development public feature set and Rams-inspired visual system from clean `wga-visual-overhaul` commit `016cf6f0e93e88ce173bff3f9e8a09d854e52d35`, including its system-font-only typography, readable relative type scale, named muted/faint/control-border and independent action-link roles, eleven independently remembered palettes comprising nine light/dark pairs and two dark-only palettes (`baroque` and `tokyo`), consistent 48px primary controls, eight-pixel spacing rhythm, tint-based record-card feedback, state-aware workspace-chip feedback, explicit keyboard/focus semantics, and reference-specific responsive composition, with pixel-accurate Chrome reference rendering at 390px, 834px, and 1440px viewports.
...
- Deliver the full public collection experience: home and navigation; artist, artwork, and selection records; an artist index and Dual Mode index with complete alphabetical school and period selects; counted multi-select artwork search with itinerary and Study Board actions; artwork and artist search loading states; inspiration; timeline; Dual Mode windows that retain complete record and curated-selection content without an image-size switch; the URL-shareable Study Board; glossary; statistics; guided tours; and reference pages.
...
- Deliver visitor participation: one-postcard delivery to as many as five deduplicated recipients through one shared recipient page, without a sender-controlled period-music choice; guestbook signing and browsing; direct project issue-tracker feedback; anonymous itineraries; published itinerary slideshows; and the persistent itinerary tray.
...
- Release acceptance covers Chrome visual comparison plus the functional and accessibility checks that can run deterministically on the Linux development machine: current stable Chrome, Playwright-managed Firefox, Chromium Android-device emulation, keyboard-only operation, semantic accessibility contracts, enlarged text/reflow, reduced motion, and no-JavaScript behaviour. Physical-device and screen-reader certification is outside this change.
