...
- Adopt the complete non-development public feature set and Rams-inspired visual system from clean `wga-visual-overhaul` commit `c09d13a5f80cdc8a980c07673bc26a4cf0e7b8cc`, including its system-font-only typography, readable relative type scale, named muted/faint/control-border and independent action-link roles, eleven independently remembered palettes comprising nine light/dark pairs and two dark-only palettes (`baroque` and `tokyo`), consistent 48px primary controls, eight-pixel spacing rhythm, tint-based record-card feedback, state-aware workspace-chip feedback, explicit keyboard/focus semantics, and reference-specific responsive composition, with pixel-accurate Chrome reference rendering at 390px, 834px, and 1440px viewports.
