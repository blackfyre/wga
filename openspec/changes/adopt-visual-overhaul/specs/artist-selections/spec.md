...
The system SHALL render each selection at a stable artist-and-selection route derived from the producer's deterministic selection identity, with its lede, commentary, selected works, links to the artist's other selections, a route to the wider holding, and a citation for the editorial text.

#### Scenario: Scholar opens a selection
- **WHEN** a scholar follows a selection route
- **THEN** the page distinguishes the selected argument from the full catalogue holding and provides a copyable citation for that selection.
