...
// annotateTextNode scans a text node for glossary terms and replaces the first
// occurrence of each unmatched term with annotated HTML.
func annotateTextNode(textNode *html.Node, entries []GlossaryEntry, matched map[string]bool) {
	...
	for i := range entries {
		...
		// Split the text node into: before + annotated span + after
		before := text[:idx]
		term := text[idx : idx+len(entry.MatchTerm)]
		after := text[idx+len(entry.MatchTerm):]
		...
		span := &html.Node{
			...
			Attr: []html.Attribute{
				...
				{Key: "data-glossary-def", Val: entry.Definition},
				{Key: "tabindex", Val: "0"},
			},
		}
		span.AppendChild(&html.Node{Type: html.TextNode, Data: term})
		parent.InsertBefore(span, textNode)

		// Update current text node to contain only "after"
		textNode.Data = after

		// If there's remaining text, continue scanning it for other terms
		if after != "" {
